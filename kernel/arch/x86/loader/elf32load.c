...
bool elf32_execute(
    ...
    for (uintptr_t x = 0;
         x < (uint32_t) header->e_phentsize * header->e_phnum;
    ...
    for (uintptr_t x = 0;
        ...
        Elf32_Phdr* phdr = (Elf32_Phdr*) (bytes + header->e_phoff + x);
        if (phdr->p_type == PT_LOAD) {
            for (uintptr_t i = phdr->p_vaddr;
                 i < phdr->p_vaddr + phdr->p_memsz;
                 i += 0x1000) {
                /* This doesn't care if we already allocated this page */
                paging_allocate_frame(
                    paging_get_page(i, 1, paging_get_directory()),
                    0, 1
                );
    ...
    for (uintptr_t stack_pointer = USER_STACK_BOTTOM;
    ...
    paging_allocate_frame(
        paging_get_page(heap, 1, paging_get_directory()),
        0, 1
    );
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        for (uintptr_t x = heap; x < heap + argsz + 0x1000; x += 0x1000) {
            paging_allocate_frame(
                paging_get_page(x, 1, paging_get_directory()),
                0, 1
            );
        }
        paging_invalidate_tables_at(heap);
    ...
    paging_allocate_frame(
        paging_get_page(process->image.heap_actual, 1, paging_get_directory()),
        0, 1
    );
