...
static void set_frame(uintptr_t frameAddr) {
    uintptr_t frame = frameAddr / PAGE_SIZE;
...
uintptr_t paging_memory_total(void) {
    return 4 * frame_count * PAGE_SIZE;
}
...
static uintptr_t map_to_physical(uintptr_t virtual) {
    uintptr_t remaining = virtual % PAGE_SIZE;
    uintptr_t frame = virtual / PAGE_SIZE;
    ...
    if (current_directory->tables[table]) {
        ...
        return p->frame * PAGE_SIZE + remaining;
    }
...
static uintptr_t first_n_frames(int n) {
    for (uintptr_t i = 0; i < frame_count * PAGE_SIZE; i += PAGE_SIZE) {
        int bad = 0;

        for (int j = 0; j < n; j++) {
            if (test_frame(i + PAGE_SIZE * j)) {
                bad = j + 1;
            }
        }
...
uintptr_t paging_allocate_aligned_large(uintptr_t address, size_t size, uintptr_t* phys) {
    for (uintptr_t i = address; i < address + size; i += PAGE_SIZE) {
        clear_frame(map_to_physical(i));
    }

    spin_lock(&frame_alloc_lock);
    uintptr_t index = first_n_frames((size + 0xFFF) / PAGE_SIZE);
    if (index == 0xFFFFFFFF) {
        spin_unlock(&frame_alloc_lock);
        return 0;
    }

    for (uint i = 0; i < (size + 0xFFF) / PAGE_SIZE; i++) {
        set_frame((index + i) * PAGE_SIZE);
...
void paging_finalize(void) {
    ...
    */
    for (uintptr_t i = PAGE_SIZE; i < 0x80000; i += PAGE_SIZE) {
        dma_frame(paging_get_page(i, 1, kernel_directory), 1, 0, i);
    }

    for (uintptr_t i = 0x80000; i < 0x100000; i += PAGE_SIZE) {
        dma_frame(paging_get_page(i, 1, kernel_directory), 1, 0, i);
    }
    ...
    */
    for (uintptr_t i = 0x100000; i < kp_placement_pointer + 0x3000; i += PAGE_SIZE) {
        dma_frame(paging_get_page(i, 1, kernel_directory), 1, 0, i);
    }

    /* Allocate frames for VGA text-mode. */
    for (uintptr_t j = 0xb8000; j < 0xc0000; j += PAGE_SIZE) {
    ...
    for (uintptr_t i = tmp_heap_start; i < KERNEL_HEAP_END; i += PAGE_SIZE) {
...
void paging_map_dma(uintptr_t virt, uintptr_t phys) {
    ...
    if (page == NULL) {
        printf(WARN "[DMA] Failed to map 0x%x to 0x%x.\n", virt, phys);
        return;
    }
...
uintptr_t paging_add_map(uintptr_t physical, size_t size) {
    ...
    if ((physical & PAGE_OFFSET_MASK) != 0) {
        offset = (physical & PAGE_OFFSET_MASK);
        size += offset;
    }

    size_t pages = (size + PAGE_OFFSET_MASK) / PAGE_SIZE;
    ...
    for (size_t idx = 0; idx < pages; idx++) {
        paging_map_dma(ptr + (PAGE_SIZE * idx), physical + (PAGE_SIZE * idx));
    }
...
//    uintptr_t phys = paging_get_physical_address(logical);
//
//    if ((logical & PAGE_OFFSET_MASK) != 0) {
//        size += logical & PAGE_OFFSET_MASK;
//        logical &= ~PAGE_OFFSET_MASK;
//    }
//
//    size_t pages = (size + PAGE_SIZE - 1) / PAGE_SIZE;
//
//    for (size_t idx = 0; idx < pages; idx++) {
//        uintptr_t addr = (logical + (PAGE_SIZE * idx));
//    }
//
//    paging_invalidate_tables();
}
