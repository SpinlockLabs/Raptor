...
boolean DWHCIRootPortInitialize (TDWHCIRootPort *pThis)
{
	...
	{
        UspiLogWrite(FromDWHCIRoot, LOG_ERROR, "Cannot detect port speed");
    ...
    UspiLogWrite(FromDWHCIRoot, LOG_NOTICE, "Device %s found", StringGet(pNames));
	...
	if (pChild != 0)
	{
		...
		{
            UspiLogWrite(FromDWHCIRoot, LOG_ERROR, "Cannot configure device");
        ...
        UspiLogWrite(FromDWHCIRoot, LOG_DEBUG, "Device configured");
	}
	else
	{
        UspiLogWrite(FromDWHCIRoot, LOG_NOTICE, "Device is not supported");
	...
	{
        UspiLogWrite(FromDWHCIRoot, LOG_ERROR, "Over-current condition");
