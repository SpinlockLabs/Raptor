...
#define ARM_IRQ_USB		9		// for UspiConnectInterrupt()

#define DEVICE_ID_USB_HCD	3		// for UspiSetPowerStateOn()
...
boolean DWHCIDeviceInitialize (TDWHCIDevice *pThis)
{
	...
	{
		UspiLogWrite(FromDWHCI, LOG_ERROR, "Unknown vendor 0x%x", DWHCIRegisterGet(&VendorId));
		_DWHCIRegister (&VendorId);
		return FALSE;
	}

	if (!UspiSetPowerStateOn(DEVICE_ID_USB_HCD))
	{
		UspiLogWrite(FromDWHCI, LOG_ERROR, "Cannot power on");
		_DWHCIRegister (&VendorId);
		return FALSE;
	}
	...
	UspiConnectInterrupt(ARM_IRQ_USB, DWHCIDeviceInterruptHandler, pThis);

	if (!DWHCIDeviceInitCore (pThis))
	{
		UspiLogWrite(FromDWHCI, LOG_ERROR, "Cannot initialize core");
	...
	{
		UspiLogWrite(FromDWHCI, LOG_ERROR, "Cannot initialize host");
		_DWHCIRegister (&AHBConfig);
		_DWHCIRegister (&VendorId);
		return FALSE;
	}
	...
	{
		UspiLogWrite(FromDWHCI, LOG_WARNING, "No device connected to root port");
		_DWHCIRegister (&AHBConfig);
		_DWHCIRegister (&VendorId);
		return TRUE;
	}
	...
	{
		UspiLogWrite(FromDWHCI, LOG_WARNING, "Cannot initialize root port");
		_DWHCIRegister (&AHBConfig);
		_DWHCIRegister (&VendorId);
		return TRUE;
	}
...
boolean DWHCIDeviceSetAddress (TDWHCIDevice *pThis, TUSBEndpoint *pEndpoint, u8 ucDeviceAddress)
{
	...
	UspiMsDelay(50);		// see USB 2.0 spec (tDSETADDR)
...
boolean DWHCIDeviceInitCore (TDWHCIDevice *pThis)
{
	...
	{
		UspiLogWrite(FromDWHCI, LOG_ERROR, "Reset failed");
		return FALSE;
	}
...
boolean DWHCIDeviceEnableRootPort (TDWHCIDevice *pThis)
{
	...
	UspiMsDelay(100);			// see USB 2.0 spec
	...
	UspiMsDelay(50);			// see USB 2.0 spec (tDRSTR)
	...
	UspiMsDelay(20);			// see USB 2.0 spec (tRSTRCY)
...
void DWHCIDeviceFlushTxFIFO (TDWHCIDevice *pThis, unsigned nFIFO)
{
	...
	{
		UspiUsDelay(1);		// Wait for 3 PHY clocks
	}
...
void DWHCIDeviceChannelInterruptHandler (TDWHCIDevice *pThis, unsigned nChannel)
{
	...
	case StageStateNoSplitTransfer:
		...
		{
			UspiLogWrite(FromDWHCI, LOG_ERROR, "Transaction failed (status 0x%X)", nStatus);
			 ...
			 && DWHCITransferStageDataIsPeriodic (pStageData))
		{
			...
			UspiStartKernelTimer(MSEC2HZ (nInterval), DWHCIDeviceTimerHandler, pStageData, pThis);
...
boolean DWHCIDeviceWaitForBit (TDWHCIDevice *pThis, TDWHCIRegister *pRegister, u32 nMask, boolean bWaitUntilSet, unsigned nMsTimeout)
{
	...
	while ((DWHCIRegisterRead (pRegister) & nMask) ? !bWaitUntilSet : bWaitUntilSet)
	{
		...
		if (--nMsTimeout == 0)
		{
			//UspiLogWrite (FromDWHCI, LOG_WARNING, "Timeout");
#ifndef NDEBUG
...
void DWHCIDeviceDumpRegister (TDWHCIDevice *pThis, const char *pName, u32 nAddress)
{
	...
	UspiLogWrite(FromDWHCI, LOG_DEBUG, "0x%08X %s", DWHCIRegisterRead(&Register), pName);
