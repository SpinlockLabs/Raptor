...
boolean USBBulkOnlyMassStorageDeviceConfigure (TUSBDevice *pUSBDevice)
{
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "Cannot set configuration");
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "Device does not respond");
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "Unsupported device type: 0x%02X",
	...
	while (--nTries)
	{
		...
		if (USBBulkOnlyMassStorageDeviceCommand (pThis, &SCSIRequestSense, sizeof SCSIRequestSense,
							 ...
							 TRUE) < 0)
		{
            UspiLogWrite(FromUmsd, LOG_ERROR, "Request sense failed");
	...
	if (nTries == 0)
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "Unit is not ready");
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "Read capacity failed");
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "Unsupported block size: %u", nBlockSize);
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "Unsupported disk size > 2TB");
    ...
    UspiLogWrite(FromUmsd, LOG_DEBUG, "Capacity is %u MByte", pThis->m_nBlockCount / (0x100000 / UMSD_BLOCK_SIZE));
...
int USBBulkOnlyMassStorageDeviceTryRead (TUSBBulkOnlyMassStorageDevice *pThis, void *pBuffer, unsigned nCount)
{
	...
	//UspiLogWrite (FromUmsd, LOG_DEBUG, "TryRead %u/0x%X/%u", nBlockAddress, (unsigned) pBuffer, (unsigned) usTransferLength);
	...
	if (USBBulkOnlyMassStorageDeviceCommand (pThis, &SCSIRead, sizeof SCSIRead,
						 ...
						 TRUE) != (int) nCount)
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "TryRead failed");
...
int USBBulkOnlyMassStorageDeviceTryWrite (TUSBBulkOnlyMassStorageDevice *pThis, const void *pBuffer, unsigned nCount)
{
	...
	//UspiLogWrite (FromUmsd, LOG_DEBUG, "TryWrite %u/0x%X/%u", nBlockAddress, (unsigned) pBuffer, (unsigned) usTransferLength);
	...
	if (USBBulkOnlyMassStorageDeviceCommand (pThis, &SCSIWrite, sizeof SCSIWrite,
						 ...
						 FALSE) < 0)
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "TryWrite failed");
					 ...
					 void *pBuffer, unsigned nBufLen, boolean bIn)
{
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "CBW transfer failed");
	...
	if (nBufLen > 0)
	{
		...
		if (nResult < 0)
		{
            UspiLogWrite(FromUmsd, LOG_ERROR, "Data transfer failed");
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "CSW transfer failed");
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "CSW signature is wrong");
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "CSW tag is wrong");
	...
	{
        UspiLogWrite(FromUmsd, LOG_ERROR, "Data residue is not 0");
...
int USBBulkOnlyMassStorageDeviceReset (TUSBBulkOnlyMassStorageDevice *pThis)
{
	...
	{
        UspiLogWrite(FromUmsd, LOG_DEBUG, "Cannot reset device");
	...
	{
        UspiLogWrite(FromUmsd, LOG_DEBUG, "Cannot clear halt on endpoint 1");
	...
	{
        UspiLogWrite(FromUmsd, LOG_DEBUG, "Cannot clear halt on endpoint 2");
