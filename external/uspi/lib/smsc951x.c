...
boolean SMSC951xDeviceConfigure (TUSBDevice *pUSBDevice)
{
	...
	if (UspiGetMACAddress(MACAddress))
	{
		MACAddressSet (&pThis->m_MACAddress, MACAddress);
	}
	else
	{
        UspiLogWrite(FromSMSC951x, LOG_ERROR, "Cannot get MAC address");
    ...
    UspiLogWrite(FromSMSC951x, LOG_DEBUG, "MAC address is %s", StringGet(&MACString));
	...
	{
        UspiLogWrite(FromSMSC951x, LOG_ERROR, "Cannot set configuration");
	...
	{
        UspiLogWrite(FromSMSC951x, LOG_ERROR, "Cannot set MAC address");
	...
	{
        UspiLogWrite(FromSMSC951x, LOG_ERROR, "Cannot start device");
...
boolean SMSC951xDeviceReceiveFrame (TSMSC951xDevice *pThis, void *pBuffer, unsigned *pResultLength)
{
	...
	{
        UspiLogWrite(FromSMSC951x, LOG_WARNING, "RX error (status 0x%X)", nRxStatus);
	...
	//UspiLogWrite (FromSMSC951x, LOG_DEBUG, "Frame received (status 0x%X)", nRxStatus);
...
void SMSC951xDeviceDumpReg (TSMSC951xDevice *pThis, const char *pName, u32 nIndex)
{
	...
	{
        UspiLogWrite(FromSMSC951x, LOG_ERROR, "Cannot read register 0x%X", nIndex);

		return;
	}

    UspiLogWrite(FromSMSC951x, LOG_DEBUG, "%08X %s", nValue, pName);
}
