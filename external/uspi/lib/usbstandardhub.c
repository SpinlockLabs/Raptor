...
boolean USBStandardHubConfigure (TUSBDevice *pUSBDevice)
{
	...
	{
        UspiLogWrite(FromHub, LOG_ERROR, "Unsupported hub (proto %u)",
	...
	{
        UspiLogWrite(FromHub, LOG_ERROR, "Cannot set configuration");
	...
	if (pInterfaceDesc->bAlternateSetting != 0)
	{
		...
		{
            UspiLogWrite(FromHub, LOG_ERROR, "Cannot set interface");
	...
	{
        UspiLogWrite(FromHub, LOG_ERROR, "Cannot get hub descriptor");
	...
	{
        UspiLogWrite(FromHub, LOG_ERROR, "Too many ports (%u)", pThis->m_nPorts);
	...
	{
        UspiLogWrite(FromHub, LOG_ERROR, "Port enumeration failed");
...
boolean USBStandardHubEnumeratePorts (TUSBStandardHub *pThis)
{
	...
	for (unsigned nPort = 0; nPort < pThis->m_nPorts; nPort++)
	{
		...
		{
            UspiLogWrite(FromHub, LOG_ERROR, "Cannot power port %u", nPort + 1);
	...
	for (unsigned nPort = 0; nPort < pThis->m_nPorts; nPort++)
	{
		...
		{
            UspiLogWrite(FromHub, LOG_ERROR, "Cannot get status of port %u", nPort + 1);
		...
		{
            UspiLogWrite(FromHub, LOG_ERROR, "Cannot reset port %u", nPort + 1);
		...
		//UspiLogWrite (FromHub, LOG_DEBUG, "Port %u status is 0x%04X", nPort+1, (unsigned) pThis->m_pStatus[nPort]->wPortStatus);
		
		if (!(pThis->m_pStatus[nPort]->wPortStatus & PORT_ENABLE__MASK))
		{
            UspiLogWrite(FromHub, LOG_ERROR, "Port %u is not enabled", nPort + 1);
		...
		if (pThis->m_pStatus[nPort]->wPortStatus & PORT_OVER_CURRENT__MASK)
		{
            ...
            UspiLogWrite(FromHub, LOG_ERROR, "Over-current condition on port %u", nPort + 1);
        ...
        UspiLogWrite(FromHub, LOG_NOTICE, "Port %u: Device %s found", nPort + 1, StringGet(pNames));
	...
	for (unsigned nPort = 0; nPort < pThis->m_nPorts; nPort++)
	{
		...
		if (pChild != 0)
		{
			...
			{
                UspiLogWrite(FromHub, LOG_ERROR, "Port %u: Cannot configure device", nPort + 1);

				continue;
			}

            UspiLogWrite(FromHub, LOG_DEBUG, "Port %u: Device configured", nPort + 1);
		}
		else
		{
            UspiLogWrite(FromHub, LOG_NOTICE, "Port %u: Device is not supported", nPort + 1);
	...
	{
        UspiLogWrite(FromHub, LOG_ERROR, "Cannot get hub status");
	...
	if (pHubStatus->wHubStatus & HUB_OVER_CURRENT__MASK)
	{
        ...
        UspiLogWrite(FromHub, LOG_ERROR, "Hub over-current condition");
